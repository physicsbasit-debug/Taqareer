(() => {
  ...
  function analyzeMultiSubjectResults(context) {
    ...
    const subjectStats = subjectRoles.map((subject, subjectIndex) => {
      ...
      let consistencyMatches = 0, consistencyTotal = 0;
      ...
      return {
        ...
        n,
        mean: n ? round(mean(values), 2) : null,
        median: n ? round(median(values), 2) : null,
        sd: n ? round(sd(values), 2) : null,
    ...
    const originalSubjectOrder = subjectRoles.map(item => item.subject);
    const studentProfiles = rows.map((item, rowIndex) => {
    ...
    const lowThreshold = Math.ceil(subjectStats.length * .6);
    ...
    if (analysisMode === "subject") {
      ...
      result.limitations = [
        ...
        "الأسماء محفوظة محليًا في جدول الأوائل ولا تغادر المتصفح ضمن حزمة التحليل.",
    ...
    const overallMean = allScores.length ? round(mean(allScores), 2) : null;
    const overallHighCount = overallLevels["أ"] + overallLevels["ب"];
    ...
    result.limitations = [
      ...
      "الأسماء والجنسية حقول حساسة؛ تُحجب قبل المعالجة الخارجية ولا تستخدم الجنسية في المقارنة التربوية.",
  ...
  function analyzeGeneric(context) {
    ...
    result.analysisProfile.assumptions = ["نوع النموذج جديد؛ يظل معنى المؤشرات وقراراتها بحاجة إلى اعتماد المستخدم والتحقق الدلالي."];
    ...
    result.improvementPlan = [
      intervention("عالية", "اعتماد عقد النوع الجديد", "مالك النموذج والمختص التربوي", "تحديد الهدف ووحدة التحليل والمعيار والقرارات المطلوبة، ثم اعتماد خطة التحليل والأدوات الحتمية المنطبقة.", "مالك النظام والمختص التربوي", "جلسة إعداد واحدة", "حفظ عقد نوع قابل لإعادة الاستخدام وإنتاج تقرير متسق على ثلاث عينات", "مراجعة نتائج ثلاث ملفات من النوع نفسه", "تعديل تعريف الحقول أو الأدوات عند عدم اتساق النتائج.", []),
    ...
    result.executiveSummary = `بنى المحرك ملفًا تحليليًا من بنية ${context.headers.length} حقلًا دون فرض قالب نتائج أو إشراف أو استبانة. طُبقت الأدوات التي تحققت شروطها، ويستكمل التحقق الدلالي تفسير السياق وإنشاء عقد خاص بالنوع قبل الاعتماد النهائي.`;
  ...
  const analyzers = {
    ...
    multi_subject_results: analyzeMultiSubjectResults,
