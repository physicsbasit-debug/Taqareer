...
function paginatePackableRun(run){if(!run.length)return;const anchor=run[0],template=run[0],source=[];run.forEach(sheet=>{const section=sectionName(sheet),content=sheet.querySelector(".page-content");[...content.children].forEach(node=>source.push({section,node:node.cloneNode(true)}))});let current=null,pendingHeading=null,pendingSection="";
  ...
  function appendAtomic(node,section,heading=null){if(!current)fresh(section);const nodes=[heading?.cloneNode(true),node.cloneNode(true)].filter(Boolean);if(tryAppend(current,nodes)){setSheetSections(current,section);return}if(current.querySelector(".page-content").children.length)fresh(section);const retry=[heading?.cloneNode(true),node.cloneNode(true)].filter(Boolean);if(!tryAppend(current,retry))forceAppend(retry,section);else setSheetSections(current,section)}
  function appendContainer(container,section,heading=null){const items=[...container.children];if(!items.length){appendAtomic(container,section,heading);return}let fragment=null;items.forEach((item,index)=>{const first=index===0;const child=item.cloneNode(true);if(!current)fresh(section);if(!fragment){fragment=refreshFragment(container.cloneNode(false));fragment.replaceChildren(child);const label=first&&heading?heading.cloneNode(true):(!first?continuationLabel(heading,section):null);if(!tryAppend(current,[label,fragment])){fresh(section);fragment=refreshFragment(container.cloneNode(false));fragment.replaceChildren(item.cloneNode(true));const retryLabel=first&&heading?heading.cloneNode(true):continuationLabel(heading,section);if(!tryAppend(current,[retryLabel,fragment]))forceAppend([retryLabel,fragment],section)}setSheetSections(current,section);return}fragment.appendChild(child);refreshFragment(fragment);void current.offsetHeight;if(pageFits(current)){setSheetSections(current,section);return}child.remove();refreshFragment(fragment);fresh(section);fragment=refreshFragment(container.cloneNode(false));fragment.replaceChildren(item.cloneNode(true));const retryLabel=continuationLabel(heading,section);if(!tryAppend(current,[retryLabel,fragment]))forceAppend([retryLabel,fragment],section);setSheetSections(current,section)})}
