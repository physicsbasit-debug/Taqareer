(() => {
  ...
  function rankingPages(privateTables){
    ...
    Object.entries(privateTables.subjectTopTen||{}).forEach(([subject,rows])=>{
      ...
      chunks.forEach((chunk,index)=>blocks.push({subject,rows:chunk,part:index+1,totalParts:chunks.length}));
    });
    let group=[],budget=0;
    const flush=()=>{
      if(!group.length)return;
      pages.push({kind:"subject-ranking",section:"الأوائل حسب المادة",content:`<div class="section-heading"><span>ترتيب المادة</span><h2>العشرة الأوائل حسب الدرجة</h2><p>تظهر جميع حالات التعادل التي تقع داخل المركز العاشر، وتُقسم الجداول الطويلة تلقائيًا دون إسقاط أي اسم.</p></div><div class="ranking-stack ranking-stack-${group.length}">${group.map(block=>rankingTable(block.totalParts>1?`${block.subject} - متابعة ${block.part}/${block.totalParts}`:block.subject,block.rows,columns,block.totalParts>1?`الجزء ${block.part} من ${block.totalParts}`:"")).join("")}</div>`});
      group=[];budget=0;
    };
    blocks.forEach(block=>{
      const cost=block.rows.length+3;
      const splitSeries=block.totalParts>1;
      if(splitSeries){
        flush();
        group=[block];budget=cost;flush();
        return;
      }
      if(group.length&&(group.length>=3||budget+cost>48))flush();
      group.push(block);budget+=cost;
    });
    flush();
    return pages;
  }

  function pageShell(content,{section,pageNo,totalPages,reportMode}){
  ...
  function buildReportHtml(context,options={}){
    ...
    const body=outputPages.map((page,index)=>pageShell(page.content,{section:page.section,pageNo:index+1,totalPages:total,reportMode})).join("");
...
.ranking-stack{display:block}.ranking-report-block{margin:2.4mm 0;border:1px solid var(--line);border-radius:2mm;overflow:hidden;break-inside:avoid}.ranking-report-head{display:flex;justify-content:space-between;gap:3mm;padding:1.7mm 2.3mm;background:#f4f7fb}.ranking-report-head h3{margin:0;color:var(--navy);font-size:8.8pt}.ranking-report-head span{font-size:6.4pt;color:var(--muted)}.ranking-report-table{width:100%;border-collapse:collapse;font-size:6.7pt;line-height:1.14}.ranking-report-table th,.ranking-report-table td{padding:1.05mm 1.5mm;border-bottom:1px solid var(--line);text-align:right;vertical-align:middle}.ranking-report-table th{background:#fafbfc;color:var(--navy);font-size:6.5pt}.ranking-report-table td:first-child{font-weight:900;color:var(--royal)}.ranking-dense .ranking-report-head{padding:1.35mm 2mm}.ranking-dense .ranking-report-head h3{font-size:8.2pt}.ranking-dense .ranking-report-table{font-size:6.15pt;line-height:1.05}.ranking-dense .ranking-report-table th,.ranking-dense .ranking-report-table td{padding:.72mm 1.25mm}.ranking-stack-3 .ranking-report-block{margin:1.8mm 0}.ranking-stack-3 .ranking-report-head{padding:1.3mm 2mm}.ranking-stack-3 .ranking-report-table th,.ranking-stack-3 .ranking-report-table td{padding:.82mm 1.35mm}.report-note{margin-top:2mm;padding:2mm;background:#fff8e9;border:1px solid #ecd9a4;font-size:7pt;color:#6d520f}
    ...
    </style></head><body><div class="toolbar"><span class="report-mode-badge">${reportMode==="executive"?"التقرير التنفيذي المختصر":"التقرير الكامل مع الجداول"}</span><button class="print" onclick="window.print()">طباعة التقرير أو حفظه PDF - متوافق A4 وLetter</button><button class="close" onclick="window.close()">إغلاق</button></div>${body}<script>window.addEventListener("load",()=>setTimeout(()=>{const failures=[];document.querySelectorAll(".chart-card[data-expected-rows]").forEach(card=>{const expected=Number(card.dataset.expectedRows||0),rows=[...card.querySelectorAll(".bar-row")],cardBox=card.getBoundingClientRect(),page=card.closest(".report-page"),footer=page?.querySelector(".page-footer"),limit=footer?footer.getBoundingClientRect().top:cardBox.bottom;if(rows.length!==expected)failures.push("count:"+(card.dataset.chartId||"")+":"+rows.length+"/"+expected);const last=rows.at(-1);if(last&&last.getBoundingClientRect().bottom>Math.min(cardBox.bottom,limit)+1)failures.push("clip:"+(card.dataset.chartId||""));});document.querySelectorAll(".ranking-report-block").forEach((block,index)=>{const page=block.closest(".report-page"),footer=page?.querySelector(".page-footer");if(footer&&block.getBoundingClientRect().bottom>footer.getBoundingClientRect().top-1)failures.push("ranking-clip:"+(index+1));});document.documentElement.dataset.chartIntegrity=failures.length?"fail":"pass";document.documentElement.dataset.chartIntegrityDetails=failures.join(",");if(failures.length)console.error("Taqareer chart integrity failed",failures);},80));<\/script>${autoPrint?`<script>window.addEventListener("load",()=>setTimeout(()=>window.print(),350));<\/script>`:""}</body></html>`;
  }
