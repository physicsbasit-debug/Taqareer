(() => {
  ...
  function reportSourceLabel(context={}){
    const sourceMeta=context.sourceMeta&&typeof context.sourceMeta==="object"?context.sourceMeta:{};
    const raw=String(context.sourceName||"").trim();
    ...
    if(["image","png","jpg","jpeg","webp"].includes(sourceType)||/\.(?:png|jpe?g|webp)(?:\s|$|[·•])/i.test(raw))return "صورة مرفوعة";
    let clean=raw.replace(/\s*[·•]\s*(?:جدول PDF منظم|الصفحات?\s*[0-9،,\-–— ]+|صفحة\s*\d+).*$/i,"").replace(/\s*[·•]\s*(?:sheet|ورقة)\s*\d+.*$/i,"").trim();
    return safePublicText(clean||"ملف مرفوع","ملف مرفوع");
  }
  ...
  function planCard(item,index){const high=normalize(item.priority).includes("عالي"),steps=item.implementationSteps||[],resources=item.resources||[],basis=item.basisClaimType?`<div class="plan-basis"><strong>${escapeHtml(claimTypeLabel(item.basisClaimType))}</strong>${item.basisConfidence?` · ثقة ${escapeHtml(item.basisConfidence)}`:""}${item.guardedConstructs?.length?` · ${escapeHtml(item.guardedConstructs.join("، "))}`:""}</div>`:"";return `<article class="plan-card"><header><span class="plan-index">${index}</span><div><small>${escapeHtml(item.priority)}</small><h3>${escapeHtml(item.issue)}</h3></div><b class="priority${high?" high":""}">${escapeHtml(item.targetGroup)}</b></header>${basis}<p class="plan-action">${escapeHtml(item.action)}</p>${steps.length?`<div class="implementation-steps"><span>خطوات التنفيذ</span><ol>${steps.map(step=>`<li>${escapeHtml(step)}</li>`).join("")}</ol></div>`:""}<div class="plan-grid"><div><span>المسؤول</span><strong>${escapeHtml(item.responsibleRole)}</strong></div><div><span>الإطار الزمني</span><strong>${escapeHtml(item.timeframe)}</strong></div><div><span>مؤشر النجاح</span><strong>${escapeHtml(item.successIndicator)}</strong></div><div><span>المتابعة</span><strong>${escapeHtml(item.monitoringMethod)}</strong></div></div>${resources.length?`<div class="resources"><span>الموارد</span>${escapeHtml(resources.join("، "))}</div>`:""}<div class="contingency"><span>الخطة البديلة</span>${escapeHtml(item.contingency)}</div></article>`;}
  ...
  function buildReportHtml(context,options={}){
    const data=buildReportData(context),analysis=data.analysis,meta=data.meta,autoPrint=options.autoPrint===true,reportMode=options.reportMode==="executive"?"executive":"full",metricGroups=selectMetricGroups(data),title=reportTitle(data,context),sourceDisplay=reportSourceLabel(context),subtitle=clampText(meta.title||sourceDisplay,240);
    ...
    const isMultiSubject=context.type?.id==="multi_subject_results";
    const multiSubjectScope=analysis.scopeContext?.analysisMode==="subject"?"مادة واحدة":"تحليل شامل";
    const metaEntries=isMultiSubject
      ? [["نوع المصدر","كشف نتائج متعدد المواد"],["نطاق التحليل",multiSubjectScope],["المدرسة",meta.school||"غير محددة في الملف"],["المادة",meta.subject||"غير محددة"],["الصف / الفئة",meta.grade||"غير محدد"],["العام / الفترة",[meta.academicYear,meta.period].filter(Boolean).join(" - ")||"غير محدد"],["مصدر البيانات",sourceDisplay]]
...
function publicFlowSectionName(value){const names={"الاستنتاجات التشخيصية":"الاستنتاجات","خطة التحسين والتدخل":"خطة التحسين","المتابعة والحوكمة":"المتابعة","القراءة التفسيرية":"القراءة التفسيرية","التحليل المتقدم":"التحليل المتقدم","الرسوم المتخصصة":"الرسوم المتخصصة"};return names[value]||value}
function setSheetSections(sheet,section){const values=(sheet.dataset.flowSections||"").split("|").map(value=>value.trim()).filter(Boolean);if(section&&!values.includes(section))values.push(section);sheet.dataset.flowSections=values.join("|");sheet.dataset.section=values.join(" / ");const label=sheet.querySelector(".page-section small");if(label)label.textContent=(values.map(publicFlowSectionName).join(" / ")||sectionName(sheet))}
