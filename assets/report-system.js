...
.report-section-bridge{height:3mm;margin:2.8mm 0 1.5mm;border-top:1px dashed #cbd4e2}.report-sheet-auto-packed .section-heading{margin-top:1.5mm;margin-bottom:1.5mm}.report-sheet-auto-packed .report-section-bridge+.section-heading{margin-top:.5mm}.report-sheet-auto-packed .finding-card{min-height:0}.report-sheet-auto-packed .finding-card>div:last-child{padding:2.5mm}.report-sheet-auto-packed .finding-card>div>p{margin:1.2mm 0;line-height:1.38}.report-sheet-auto-packed .finding-card dl div{padding-top:.9mm;margin-top:.9mm}.report-sheet-auto-packed .finding-card dd{line-height:1.32}.report-sheet-auto-packed .plan-card{min-height:0;padding:2.5mm}.report-sheet-auto-packed .plan-action{margin:1.5mm 0;line-height:1.38}.report-sheet-auto-packed .implementation-steps,.report-sheet-auto-packed .resources,.report-sheet-auto-packed .contingency{margin:1.2mm 0;padding:1.4mm;line-height:1.28}.report-sheet-auto-packed .plan-grid div{padding:1.6mm}.report-sheet-auto-packed .plan-cards{gap:2.7mm}.report-sheet-auto-packed .plan-card h3,.report-sheet-auto-packed .finding-card h3{font-size:9.8pt}.analytical-charts:not(:has(.chart-wide)) .chart-card{min-height:104mm}.ranking-stack{display:block}.ranking-companion{margin-top:4mm;padding-top:3mm;border-top:1px dashed #cbd4e2}.ranking-companion-title{display:flex;justify-content:space-between;align-items:center;gap:3mm;margin-bottom:1.5mm}.ranking-companion-title span{font-size:7pt;color:var(--muted);font-weight:800}.ranking-companion-title strong{font-size:8.6pt;color:var(--navy)}.ranking-stack-companion .ranking-report-block{margin:0}.ranking-report-block{margin:2.4mm 0;border:1px solid var(--line);border-radius:2mm;overflow:hidden;break-inside:avoid}.ranking-report-head{display:flex;justify-content:space-between;gap:3mm;padding:1.7mm 2.3mm;background:#f4f7fb}.ranking-report-head h3{margin:0;color:var(--navy);font-size:8.8pt}.ranking-report-head span{font-size:6.4pt;color:var(--muted)}.ranking-report-table{width:100%;border-collapse:collapse;font-size:6.7pt;line-height:1.14}.ranking-report-table th,.ranking-report-table td{padding:1.05mm 1.5mm;border-bottom:1px solid var(--line);text-align:right;vertical-align:middle}.ranking-report-table th{background:#fafbfc;color:var(--navy);font-size:6.5pt}.ranking-report-table td:first-child{font-weight:900;color:var(--royal)}.ranking-dense .ranking-report-head{padding:1.35mm 2mm}.ranking-dense .ranking-report-head h3{font-size:8.2pt}.ranking-dense .ranking-report-table{font-size:6.15pt;line-height:1.05}.ranking-dense .ranking-report-table th,.ranking-dense .ranking-report-table td{padding:.72mm 1.25mm}.ranking-stack-3 .ranking-report-block{margin:1.8mm 0}.ranking-stack-3 .ranking-report-head{padding:1.3mm 2mm}.ranking-stack-3 .ranking-report-table th,.ranking-stack-3 .ranking-report-table td{padding:.82mm 1.35mm}.ranking-stack-packed .ranking-report-block{margin:1.35mm 0}.ranking-stack-packed .ranking-report-head{padding:1.05mm 1.8mm}.ranking-stack-packed .ranking-report-head h3{font-size:8pt}.ranking-stack-packed .ranking-report-table{font-size:6pt;line-height:1.02}.ranking-stack-packed .ranking-report-table th,.ranking-stack-packed .ranking-report-table td{padding:.62mm 1.1mm}.ranking-stack-split .ranking-report-block{margin-top:1.4mm}.report-note{margin-top:2mm;padding:2mm;background:#fff8e9;border:1px solid #ecd9a4;font-size:7pt;color:#6d520f}
    ...
    </style></head><body><div class="toolbar"><span class="report-mode-badge">${reportMode==="executive"?"التقرير التنفيذي المختصر":"التقرير الكامل مع الجداول"}</span><span class="report-mode-badge">A4 · 210 × 297 مم</span><span class="report-print-hint">عند استخدام طابعة PDF اختر حجم الورق A4</span><button class="print" onclick="window.print()">طباعة التقرير أو حفظه PDF - A4</button><button class="close" onclick="window.close()">إغلاق</button></div>${body}<script>(()=>{function sectionName(sheet){return sheet?.dataset.section||sheet?.querySelector(".page-section small")?.textContent?.trim()||""}function pageFits(sheet){const content=sheet?.querySelector(".page-content"),footer=sheet?.querySelector(".page-footer");if(!content||!footer)return false;const footerTop=footer.getBoundingClientRect().top;if(content.scrollHeight>content.clientHeight+2)return false;return [...content.children].every(node=>node.getBoundingClientRect().bottom<=footerTop-2)}function updatePagination(){const sheets=[...document.querySelectorAll(".report-sheet")],total=sheets.length;sheets.forEach((sheet,index)=>{const label=sheet.querySelector(".page-footer strong");if(label)label.textContent="صفحة "+(index+1)+" من "+total})}function repackA4(){let changed=true,guard=0;while(changed&&guard++<20){changed=false;const sheets=[...document.querySelectorAll(".report-sheet")];for(let i=0;i<sheets.length-1;i++){const current=sheets[i],next=sheets[i+1];if(current.dataset.packable!=="1"||next.dataset.packable!=="1")continue;const content=current.querySelector(".page-content"),nextContent=next.querySelector(".page-content");if(!content||!nextContent)continue;const added=[];const bridge=document.createElement("div");bridge.className="report-section-bridge";bridge.setAttribute("aria-hidden","true");content.appendChild(bridge);added.push(bridge);[...nextContent.children].forEach(node=>{const clone=node.cloneNode(true);content.appendChild(clone);added.push(clone)});current.classList.add("report-sheet-auto-packed");void current.offsetHeight;if(!pageFits(current)){added.forEach(node=>node.remove());current.classList.remove("report-sheet-auto-packed");continue}const sections=[sectionName(current),sectionName(next)].filter(Boolean);current.dataset.section=sections.join(" / ");const sectionLabel=current.querySelector(".page-section small");if(sectionLabel)sectionLabel.textContent=current.dataset.section;next.remove();changed=true;break}}updatePagination();document.documentElement.dataset.a4Pagination="ready"}function integrity(){const failures=[];document.querySelectorAll(".report-sheet").forEach((sheet,index)=>{if(!pageFits(sheet))failures.push("page-overflow:"+(index+1))});document.querySelectorAll(".chart-card[data-expected-rows]").forEach(card=>{const expected=Number(card.dataset.expectedRows||0),rowClass=card.dataset.rowClass||"bar-row",rows=[...card.querySelectorAll("."+rowClass)],cardBox=card.getBoundingClientRect(),page=card.closest(".report-page"),footer=page?.querySelector(".page-footer"),limit=footer?footer.getBoundingClientRect().top:cardBox.bottom;if(rows.length!==expected)failures.push("count:"+(card.dataset.chartId||"")+":"+rows.length+"/"+expected);const last=rows.at(-1);if(last&&last.getBoundingClientRect().bottom>Math.min(cardBox.bottom,limit)+1)failures.push("clip:"+(card.dataset.chartId||""))});document.querySelectorAll(".ranking-report-block").forEach((block,index)=>{const page=block.closest(".report-page"),footer=page?.querySelector(".page-footer");if(footer&&block.getBoundingClientRect().bottom>footer.getBoundingClientRect().top-1)failures.push("ranking-clip:"+(index+1))});document.querySelectorAll(".plan-card").forEach((card,index)=>{const page=card.closest(".report-page"),footer=page?.querySelector(".page-footer");if(footer&&card.getBoundingClientRect().bottom>footer.getBoundingClientRect().top-1)failures.push("plan-clip:"+(index+1))});document.documentElement.dataset.chartIntegrity=failures.length?"fail":"pass";document.documentElement.dataset.chartIntegrityDetails=failures.join(",");if(failures.length)console.error("Taqareer chart integrity failed",failures)}window.addEventListener("load",()=>setTimeout(()=>{repackA4();requestAnimationFrame(()=>requestAnimationFrame(integrity))},90))})();<\/script>${autoPrint?`<script>window.addEventListener("load",()=>setTimeout(()=>window.print(),350));<\/script>`:""}</body></html>`;
  }
