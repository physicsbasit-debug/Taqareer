(() => {
  ...
  function pageShell(content,{section,pageNo,totalPages,code,reportMode}){
    const modeLabel=reportMode==="executive"?"تقرير تنفيذي مختصر":"تقرير تحليلي كامل";
    return `<section class="report-sheet"><div class="report-page"><header class="page-head"><div class="page-brand"><div class="mini-logo">ت</div><div><strong>تقارير</strong><span>منصة التحليل التربوي الذكي</span></div></div><div class="page-section"><small>${escapeHtml(section)}</small><b>${escapeHtml(code)}</b></div></header><div class="page-content">${content}</div><footer class="page-footer"><span>تقارير v${VERSION} - ${modeLabel} قابل للمراجعة</span><strong>صفحة ${pageNo} من ${totalPages}</strong></footer></div></section>`;
  }
  ...
  function buildReportHtml(context,options={}){
    ...
    const body=outputPages.map((page,index)=>pageShell(page.content,{section:page.section,pageNo:index+1,totalPages:total,code,reportMode})).join("");
