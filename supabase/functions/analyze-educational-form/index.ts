...
const PRIMARY_MODEL_FALLBACKS = Object.freeze(["gemini-3.5-flash", "gemini-3.6-flash", "gemini-3.5-flash-lite"]);
const PRIMARY_RESCUE_MODELS = Object.freeze(["gemini-3.5-flash-lite", "gemini-3.1-flash-lite", "gemini-3.5-flash"]);
const MAX_REQUEST_BYTES = 9_000_000;
const MAX_IMAGE_COUNT = 4;
const MAX_IMAGE_DATA_URL_LENGTH = 2_800_000;
const PRIMARY_ANALYSIS_DEADLINE_MS = 72_000;
const PRIMARY_MODEL_ATTEMPT_TIMEOUT_MS = 30_000;
const PRIMARY_RESCUE_ATTEMPT_TIMEOUT_MS = 24_000;
const PRIMARY_MIN_REMAINING_MS = 4_000;
...
async function analyzePrimary(payload: JsonRecord): Promise<JsonRecord> {
  ...
  let response = await geminiRequestWithFallback(
    models,
    primaryRequestBody(payload, primaryAnalysisInstructions(), "low", 8192),
    1,
    {
  ...
  if (!checked.result) {
    ...
    response = await geminiRequestWithFallback(
      rescueModels,
      primaryRequestBody(payload, primaryRescueInstructions(), "minimal", 6144),
      1,
      {
  ...
  return {
    ...
    serverTiming: {
      ...
      maxOutputTokens: rescueUsed ? 6144 : 8192,
      rescueUsed,
